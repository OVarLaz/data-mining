...
def fastdtw(x, y, radius=1, dist=None):
    ...
    '''
    #print "fastdtw(x, y, radius=1, dist=None)"
...
def __fastdtw(x, y, radius, dist):
    #print "__fastdtw(x, y, radius, dist)"
...
def __dtw(x, y, window, dist):
    ...
    while not (i == j == 0):
        ...
        i, j = D[i, j][1], D[i, j][2]
    path.reverse()
    #print (D[len_x, len_y][0], path)
    return (D[len_x, len_y][0], path)


def __reduce_by_half(x):
    #print "__reduce_by_half(x)"
    return [(x[i] + x[1+i]) / 2 for i in range(0, len(x) - len(x) % 2, 2)]


def __expand_window(path, len_x, len_y, radius):
    #print "__expand_window"
    path_ = set(path)
    for i, j in path:
        for a, b in ((i + a, j + b)
