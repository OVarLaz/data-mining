...
def get_data_pp(patient,nv): # Funcion para obtener en orden de tiempo 
    ...
    cur.execute("SELECT wave FROM lala.samples WHERE  file=\""+patient +"\" order by CAST(pos_begin AS UNSIGNED)")
    ...
    for wave in res:
        ...
        cont=cont+1
    data=np.array(datos)
    cur.execute("SELECT state FROM lala.samples WHERE sensor=\"EEGPzOz\" and file=\""+patient +"\" order by CAST(pos_begin AS UNSIGNED)")
    res=cur.fetchall()
