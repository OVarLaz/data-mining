...
individuos=30
similarity_file2='similarity_eros_'+str(individuos)
...
patients2 = ["SC4032E0",  "ST7011J0", "ST7022J0", "ST7041J0", "ST7052J0", "ST7061J0", "ST7071J0", "ST7082J0", "ST7092J0", "ST7101J0", "ST7112J0", "ST7121J0", "ST7132J0", "ST7141J0", "ST7151J0", "ST7162J0", "ST7171J0", "ST7182J0", "ST7192J0", "ST7201J0", "ST7212J0", "ST7221J0", "ST7241J0"]

#"SC4052E0","SC4072E0",,"SC4121E0","SC4122E0","ST7022J0"
patientsfordtw = ["SC4031E0","SC4032E0","SC4051E0","SC4071E0","ST7132J0","ST7141J0","ST7201J0","ST7071J0"]
...
def eross(patient):
    ...
    data,labels=connection2.get_data_pp(patient,4)
    #data,labels=read_data(lab,files,3,10,3000)#connection.get_data_1v()#read_data(lab,files,1,20,3000)
    print "original", data.shape
    p_data=[ np.transpose(i) for i in data]
    p_data=np.array(p_data)
    ...
    np.savetxt(similarity_file2, eros.eros(p_data).reshape((len(p_data), 1, len(p_data)))) 
    similarity_matrix = np.loadtxt(similarity_file2)

    print  'matrix loaded' #Y_2_d = mtsne.distance_tsne(similarity_matrix, 2)  
    Y_2_s = mtsne.similarity_tsne(similarity_matrix, 2)  
    print labels
    #Y_3 = mtsne.similarity_tsne(similarity_matrix, 3)
    graph_plot.graf_with_scale(patient,'ErosP-'+patient,Y_2_s, labels)
